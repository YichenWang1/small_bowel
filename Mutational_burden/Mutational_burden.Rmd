...
knitr::opts_knit$set(root.dir = '/Users/yw2/OneDrive\ -\ University\ of\ Cambridge/PhD/small_bowel')
```
...
write.table(data,'./data/stat_summary.txt', quote=F, col.names = T, row.names = F,sep='\t')
...
# Load SBS matrix
sbs<-read.table('./data/sbs/sbs_mapped_to_branches.txt',check.names = F)
# Exclude Brunner's glands
...
# Load Indel matrix
indel<-read.table('./data/indel/indel_mapped_to_branches.txt',check.names = F)
# Exclude Brunner's glands
...
lmm.sbs.coeliac.var.intra <- lme(sbs_count_adj  ~ age , random = list(patient = pdDiag(form = ~ age-1)), weights = varIdent(form = ~1 | condition ),  data = df_regression[which(df_regression$patient != 'PD46565'),], method = "ML")
anova(lmm.sbs.coeliac.var.intra, lmm.sbs.age)
